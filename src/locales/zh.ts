...
const zh: LocaleDefinition = {
  ...
  Settings: {
    ...
    s3Sync: {
      title: 'S3 同步',
      description: '将会话与附件备份到 S3 兼容存储。',
      enabled: '启用 S3 同步',
      enabledHint: '启用后可一键备份与恢复（恢复需要重启）。',
      warning: 'S3 凭证会保存在本地。请仅在可信设备使用，并尽量使用最小权限密钥。',
      endpoint: 'Endpoint',
      endpointPlaceholder: 'https://s3.amazonaws.com（或 R2/MinIO 的 endpoint）',
      region: 'Region',
      bucket: 'Bucket',
      ...
      sessionTokenPlaceholder: '临时凭证 token',
      sessionTokenHint: '除非使用 STS 临时凭证，否则留空即可。',
      pathStyle: 'Path-Style 地址',
      pathStyleHint: 'MinIO 或部分 S3 兼容服务需要开启。',
      namespace: '同步命名空间（可选）',
      namespacePlaceholder: 'shared-team（多设备保持一致）',
      namespaceHint: '留空将使用本机 device id（推荐）。多个设备设置相同命名空间可共享备份。',
      keyPrefix: 'Key 前缀',
      testConnection: '测试',
      backupNow: '立即备份',
      restore: '恢复（重启）',
      save: '保存',
      saved: 'S3 同步设置已保存',
      saveFailed: '保存 S3 同步设置失败',
      testSuccess: 'S3 连接测试成功',
      testFailed: 'S3 连接测试失败',
      backupSuccess: '备份已上传到 S3',
      backupFailed: '备份失败',
      restoreScheduled: '恢复包已下载。重启 TalkCody 以应用恢复。',
      restoreScheduleFailed: '下载恢复包失败',
      lastBackupAt: (time) => `上次备份：${time}`,
      lastBackupNone: '暂无备份',
      errors: {
        invalidConfig: '请填写所有必填项',
        notEnabled: '尚未启用 S3 同步',
      },
    },
    apiKeys: {
      title: 'API 密钥配置',
