...
# Step 2: Find updater bundle and signature (generated by Tauri build)
echo "📦 Step 2/5: Finding updater bundle and signature..."

# Find the updater bundle (.tar.gz) generated by Tauri
# Tauri generates: {app-name}_{version}_{target}.{ext}.tar.gz
UPDATER_BUNDLE=$(find "$BUNDLE_DIR" -name "*.tar.gz" -not -name "*.sig" 2>/dev/null | head -n 1)

if [ -z "$UPDATER_BUNDLE" ]; then
    echo -e "${RED}❌ Error: Updater bundle (.tar.gz) not found in $BUNDLE_DIR${NC}"
...
# Find the signature file (.sig) generated by Tauri
UPDATER_SIG="${UPDATER_BUNDLE}.sig"

if [ ! -f "$UPDATER_SIG" ]; then
    echo -e "${RED}❌ Error: Signature file not found: $(basename "$UPDATER_SIG")${NC}"
    echo "  Make sure TAURI_SIGNING_PRIVATE_KEY environment variable is set during 'bun run tauri build'"
    exit 1
fi

echo -e "${GREEN}✓${NC} Found signature file: $(basename "$UPDATER_SIG")"
echo ""
